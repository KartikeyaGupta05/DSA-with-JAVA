...
public class Find_Distinct_Colours_25 {
    public static int[] queryResults(int limit, int[][] queries) {
        int n = queries.length; // Use length for arrays
        ...
        for (int i = 0; i < n; i++) {
            ...
            int ball = queries[i][0]; // ball number for this question - {1,2,1,3}
            int color = queries[i][1]; // colours for balls this question - {4,5,3,4}

            // Remove the previous color if the ball was already painted
            ...
            // Paint the ball with the given color
            ballColor.put(ball, color);
            colorFreq.put(color, colorFreq.getOrDefault(color, 0) + 1);

            // Add the number of unique colors so far to the result list
    ...
    public static void main(String[] args) {
        ...
        for (int i : queryResults(4, queries)) {
            System.out.print(i + " ");
        }
    }
}
